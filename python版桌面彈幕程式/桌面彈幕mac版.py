#!/usr/bin/env python3
"""
桌面彈幕 v3 — PyQt6 跨平台全桌面版 (支援 Mac 與 Windows)
  · PyQt6 QPainter 直接繪製，無縮小問題
  · 跨平台相容：Mac 採用 ToolTip 置頂，並開啟 WA_LayoutOnEntireRect 物理全螢幕
  · Windows 採用 ctypes 與 Tool 旗標確保覆蓋層完全穿透與置頂
  · QFontMetrics 精確量文字寬
...
FONT_SANS = "PingFang TC" if IS_MAC else "Microsoft JhengHei"
...
def set_click_through(hwnd):
    """讓視窗完全穿透滑鼠（僅適用於 Windows）"""
    if sys.platform != "win32":
        return # Mac 不需要/無法呼叫此 Windows API
    try:
        GWL_EXSTYLE   = -20
        ...
        print(f"Windows set_click_through error: {e}")
...
class MarqueeItem:
    def __init__(self, text, settings, sw, sh):
        ...
        self.text         = text
        self.font         = QFont(FONT_SANS, size, QFont.Weight.Bold)
...
class MarqueeOverlay(QWidget):
    def __init__(self):
        ...
        # 跨平台最上層設計：
        # macOS 上採用 ToolTip (系統提示工具級別) 具備最高優先權，能無視其他置頂視窗
        # Windows 上則使用標準 Tool 視窗，配合底層 ctypes 穿透
        if IS_MAC:
            flags |= Qt.WindowType.ToolTip
        else:
            flags |= Qt.WindowType.Tool

        # 跨平台滑鼠穿透屬性 (適用於 Qt 5.9+ 及 Qt 6，能完美解決 Mac 穿透問題)
        ...
        # Mac 專屬修正：指示視窗配置允許直接覆蓋整個物理螢幕，忽略 Dock 與上選單列限制（解救 3/4 畫面截斷問題）
        if IS_MAC:
    ...
    def show_overlay(self):
        self.show()
        self.raise_()                         # 強制拉升至最前方
        QApplication.processEvents()          # 確保 winId 已建立
        if sys.platform == "win32":
            set_click_through(self.winId())   # Windows 系統呼叫額外穿透處理
...
class FloatingIcon(QWidget):
    ...
    def __init__(self, app_ref):
        ...
        if IS_MAC:
            # macOS 下 ToolTip 級別具備更高置頂優先權，可超越開啟中的瀏覽器
            flags |= Qt.WindowType.ToolTip
        else:
            flags |= Qt.WindowType.Tool
    ...
    def paintEvent(self, event):
        ...
        danmu_font = QFont(FONT_SANS, 20, QFont.Weight.Bold)
        p.setFont(danmu_font)
        p.drawText(QRect(0, 6, s, s // 2 + 8),
                   Qt.AlignmentFlag.AlignHCenter, "彈幕")

        status_font = QFont(FONT_SANS, 9, QFont.Weight.Bold)
    ...
    def contextMenuEvent(self, event):
        ...
        menu = QMenu(self)
        menu.setStyleSheet(f"QMenu {{ font-family: '{FONT_SANS}'; font-size: 12px; }}")
    ...
    def _build_ui(self):
        ...
        def card(title=None):
            ...
            if title:
                ...
                lbl.setStyleSheet(f"color:#3b82f6;font-size:13px;font-weight:bold;"
                                  f"font-family:'{FONT_SANS}';border:none;")
                v.addWidget(lbl)
            return f, v

        def sublabel(text, parent_layout):
            l = QLabel(text)
            l.setStyleSheet(f"color:#6b7280;font-size:10px;"
                            f"font-family:'{FONT_SANS}';border:none;")
        ...
        title.setStyleSheet(f"color:#3b82f6;font-size:18px;font-weight:bold;"
                            f"font-family:'{FONT_SANS}';padding:6px 0;border:none;")
        ml.addWidget(title)
        ...
        tog_lbl.setStyleSheet(f"color:white;font-size:13px;font-weight:bold;"
                              f"font-family:'{FONT_SANS}';border:none;")
        ...
        help_text.setStyleSheet(
            f"color:#4b5563;font-size:10px;font-family:'{FONT_SANS}';border:none;")
        ...
        self._status_lbl.setStyleSheet(
            f"color:#3b82f6;font-size:10px;font-family:'{FONT_SANS}';border:none;")
        ...
        foot.setStyleSheet(f"color:#9ca3af;font-size:9px;"
                           f"font-family:'{FONT_SANS}';border:none;")
        ml.addWidget(foot)
        ml.addStretch()
...
class ToastNotice(QWidget):
    def __init__(self):
        ...
        self._lbl.setStyleSheet(
            f"color:white; font-size:12px; font-family:'{FONT_SANS}';"
